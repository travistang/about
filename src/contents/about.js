...
import { Media, Card, CardBody, Collapse, Tooltip } from "reactstrap";
const hobby = [
	{
		icon: faMusic,
		name: "Piano",
		details: `
      As a trivia, I started learning piano since I was 4, so now I should have nominally been playing for ${Utils.myAge() -
				4} years...
	...
	},
	{
      ...
      But now I haven't played it for a long time since I still can't find anyone who plays it here in Germany.
      In case you are in Munich and wish to find somebody to play badminton, feel free to contact me!
      (By the way, I know the icon is a basketball instead of a badminton, I just decided to use it because there's no badminton icon in free version of Font Awesome...
		...
		return (
			<div style={style.about.interestContainer.interest}>
				<div
					...
					style={style.about.interestContainer.interest.content}>
					<div
						className={this.iconColor}
						style={style.about.interestContainer.interest.content.icon}>
						<FontAwesomeIcon icon={hob.icon} size="2x" />
					</div>
					<div
						id={`hobby-${hob.name}`}
						style={style.about.interestContainer.interest.content.text}>
						{hob.name}
					</div>
				</div>
			</div>
...
export default {
	synopsis: (
		<p>
			I am Travis, a {Utils.myAge()}-year-old guy from Hong Kong. Currently I am
			doing my Master's degree in Informatics at Technical University of Munich.
			I will be writing my Master thesis in Sept 2019.
			<br />I have a wide range of interest. From web development to mobile
			application development, machine learning (deep learning in particular),
			networking, IoT (such as with Raspberry Pi, Arduino, ESP8266...), modeling
			and simulation, quadcopters... These are all in which I have motivation to
			learn and work on. My goal is to combine the best out of these fields into
			a few projects, that people can get the most out of it.
		</p>
	),
	sections: [
		{
			...
			content: (
				<div>
					<p>
						{" "}
						I do have other interests other than writing codes and connecting
						wires, to name a few...
					</p>
					<div style={style.about.interestContainer}>
						{hobby.map(hob => (
							<HobbyComponent hobby={hob} />
						))}
					</div>
				</div>
			)
		},
		{
			...
			content: (
				<p style={style.about.text}>
					...
					<br />
					<br />
					<b>Travis</b> is my nickname that all friends of mine call but it
					never exists in any offical documents.
					<b>Yiu Ting</b> is the name that appears on my passport / IDs but
					nobody except my parents call me so.
					<br />
					<br />
					It is very common for Hongkongers to have an unofficial english name
					and we get used to calling each other with it in every context.
					<br />
					<br />
					That's why I prefer being called <b>Travis</b>. But of course you can
					call me <b>Yiu Ting</b> if you want since it really is my name and I
					personally don't care. Just try not to call me both at the same time
